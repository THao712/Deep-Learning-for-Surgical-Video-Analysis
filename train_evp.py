...
parser.add_argument('-t', '--train', default=88, type=int, help='train batch size, default 400')#autodl上改成88
parser.add_argument('-v', '--val', default=88, type=int, help='valid batch size, default 10')#autodl上改成88
...
parser.add_argument('-w', '--work', default=8, type=int, help='num of workers to use, default 4')#autodl上改成8
...
def valMinibatch(testloader, model):
    ...
    with torch.no_grad():
        ...
        for y, t in zip(predict_phase_ant_all, gt_phase_ant_all):  # 配对成元组
...
def train_model(train_dataset, train_num_each, val_dataset, val_num_each):
    # TensorBoard
    writer = SummaryWriter('bimask_ss_pos/cholec80/stage1_32_8_40/embedding1/runs/')
    ...
    for k, v in pretrained_dict.items():
        if 'head' not in k and "prompt_generator" not in k:
            new_state_dict[k] = v

    # 从上次第18个最佳epoch开始训练
    # embed_dict = torch.load(
    #     'bimask_ss_pos/cholec80/stage1_32_8_40/embedding/evpfc_ce_epoch_19_length_1_opt_0_mulopt_1_flip_1_crop_1_batch_88_train_9840_val_8152_test_7818.pth')
    ...
    # imgs_train_path = "preproc_data/imgs_train.pkl"
    # segmaps_train_path = "preproc_data/segmaps_train.pkl"
    # with open(imgs_train_path, 'rb') as f:
    #     imgs_train = pickle.load(f)
    # with open(segmaps_train_path, 'rb') as f:
    #     segmaps_train = pickle.load(f)
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(train_loader_80):
            ...
            if i % 15 == 14:
                ...
                val_loss_phase, val_loss_phase_ant, val_corrects_phase, in_MAE_m, pMAE_m, eMAE_m = valMinibatch(
                    val_loader, model)
            ...
            if batch_progress * train_batch_size >= num_train_all:
                percent = 100.0
                print('Batch progress: %s [%d/%d]' % (str(percent) + '%', num_train_all, num_train_all))
            elif batch_progress % 100 == 0:
                percent = round(batch_progress * train_batch_size / num_train_all * 100, 2)
                print('Batch progress: %s [%d/%d]' % (str(percent) + '%', batch_progress * train_batch_size, num_train_all))
        ...
        # writer.add_scalar('validation_Batch/acc phase', float(val_corrects_phase) / float(num_val_we_use), epoch)
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                if val_progress * val_batch_size >= num_val_all:
                    percent = 100.0
                    print('Val progress: %s [%d/%d]' % (str(percent) + '%', num_val_all, num_val_all))
                elif val_progress % 50 == 0:
        ...
        with torch.no_grad():
            for data in test_loader:
                ...
                if test_progress * val_batch_size >= num_test_all:
                    percent = 100.0
                    print('test progress: %s [%d/%d]' % (str(percent) + '%', num_test_all, num_test_all))
        ...
        # torch.save(best_model_wts, "./embed_model/segformer_embed/" + base_name0 + ".pth")
        ...
        torch.save(model.state_dict(), "bimask_ss_pos/cholec80/stage1_32_8_40/embedding1/" + base_name1 + ".pth")
    writer.close()
...
def main():
    ...
    train_dataset_80, train_num_each_80, \
        ...
        test_dataset_80, test_num_each_80 = get_all_data('pathfiles/cholec80/train_val_paths_labels_32_8_40.pkl',
                                                         'pathfiles/cholec80/bimasks_ss_pos_train_val_test_32_8_40.pkl')
